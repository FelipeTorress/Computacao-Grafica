...
{// desenhando as opcoes da tela
	...
	quadrilatero(34.0, 230, 4.0, 206,false);
	quadrilatero(9, 225, 27, 210,false);
	quadrilatero(10, 224, 26, 211,false);
}

void preencherFigura(int x, int y)
{
    if(matriz[x][y]) return;
    if(x>width || x <0 || y < 0 || y> height)return;
    pushPonto(x , y, false);
    matriz[x][y] = true;
    preencherFigura((x - 1), y);
    preencherFigura((x + 1), y);
    preencherFigura(x, (y - 1));
    preencherFigura(x, (y + 1));
}
...
void desenho(int x, int  y)
{
	...
	case 6://preenchimento
		if(click1){
			int contx = 0, conty = 0;
                ponto * aux;
                aux = pontos;
                for (contx = 0; contx < width; contx++)
                {
                    for (conty = 0; conty < height; conty++)
                    {
